...
Return only the improved sentence, it is very important that you do not return anything else."
        return self.submit_prompt(initial_prompt, temperature=0.2)
    ...
    def flow(self, data: dict, flow_type: str, context=False):
        with open(f"{flow_type}.json", 'r') as f:
            flow_steps = json.load(f)
            
        cells = []
        llm_response = context
        for step in flow_steps:
            llm_response = self.generate_prompt(data=data, question=step.question, context=llm_response)
